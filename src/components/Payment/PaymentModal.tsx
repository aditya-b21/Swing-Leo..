...
export function PaymentModal({ children, onVerificationChange }: PaymentModalProps) {
  const { user } = useAuth();
  ...
  // Check payment verification status after submission
  useEffect(() => {
    if (user) {
      const checkVerification = async () => {
        ...
        } else if (!error && data && data.status === 'pending') {
          setIsVerified(false);
  ...
  const fetchPaymentSettings = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('key, value')
        .in('key', ['payment_amount', 'qr_code_url', 'upi_id']);
  ...
  const submitPayment = async () => {
    ...
    try {
      ...
      // Reset form
      setFormData({ fullName: '', email: '', utrReference: '', couponCode: '' });
            ...
            <>
              ...
              <Card className="bg-gray-900 border-blue-400/30">
                ...
                <CardContent className="text-center">
                  ...
                  <Button 
                    onClick={() => copyToClipboard(upiId || 'adityabarod807@paytm')}
                    variant="outline"
                    className="border-blue-400 text-blue-400 hover:bg-blue-400 hover:text-black"
                  >
                    <Copy className="w-4 h-4 mr-2" />
                    Copy UPI ID: {upiId || 'adityabarod807@paytm'}
                  </Button>
                </CardContent>
              </Card>
          ...
          {step === 'upload' && (
            <>
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label className="text-white">Login Email *</Label>
                  <Input
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="Enter UTR or transaction reference (optional if uploading screenshot)"
                    className="bg-gray-800 border-gray-600 text-white"
                  />
                </div>
                <div className="space-y-2">
                  <Label className="text-white">Payment Proof (QR Screenshot)</Label>
                ...
                <div className="flex gap-3">
                  ...
                  <Button 
                    ...
                    disabled={loading || !formData.fullName.trim() || !formData.email.trim() || (!formData.utrReference.trim() && !paymentProof)}
